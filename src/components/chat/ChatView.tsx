...
export function ChatView() {
    ...
    const globalChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Refresh room list to update unread counts and last message
          fetchRooms();
        }
      )
      .subscribe((status) => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // Fetch the full message with sender info using the ID from payload
  ...
  const handleStartDM = async (userId: string) => {
    try {
      ...
      const json = await res.json();
      if (res.ok && json.data) {
        // Refresh rooms and select the new/existing DM
        await fetchRooms();
        ...
        if (room) {
          ...
          // Room might not be in list yet, refetch
